...
async function runDetect(inFile, seconds, vf) {
  const { stderr } = await sh("ffmpeg", ["-y", "-ss", "0", "-t", String(seconds), "-i", inFile, "-vf", vf, "-f", "null", "-"]);
  return parseCrop(stderr);
}

// score a crop; prefer more border removed but keep sane size
function scoreCrop(c, inW, inH) {
  ...
  return removed + maxBar * 200; // weight bigger single bars a bit more
}
...
app.post("/crop", async (req, res) => {
  try {
    ...
    const { w: inW, h: inH } = await getDims(inFile);

    // Try several limits and three pipelines: normal, inverted (white bars), edges (any flat color)
    const limits = [30, 40, 50, 60, 75];
    const pipelines = (lim) => [
    ...
    for (const lim of limits) {
      for (const vf of pipelines(lim)) {
        const c = await runDetect(inFile, probeSeconds, vf);
        const s = scoreCrop(c, inW, inH);
        if (s > bestScore) { best = c; bestScore = s; }
      }
      if (bestScore > 0) break; // found a decent crop already
    }
    if (!best) return res.status(422).json({ error: "Could not detect crop" });

    // Safe crop: even dims; OPTIONAL downscale to height ~1280 for speed
    const outFile = join(dir, "out.mp4");
    const vfSafe = `${best.text},scale=trunc(iw/2)*2:trunc(ih/2)*2:flags=bicubic,scale=-2:1280`;
